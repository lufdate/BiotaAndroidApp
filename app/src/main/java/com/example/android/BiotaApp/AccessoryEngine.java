...
//                Toast. makeText ( mContext, "Encontro aplicacion..!!!" ,
	...
	public void write(byte[] data) {
		if (mAccessoryConnected && mOutputStream != null) {
			try {
				...
				mOutputStream.flush();
				Toast. makeText ( mContext, "Valor enviado" ,
						Toast . LENGTH_SHORT ). show (); // display a toast message
			} catch (IOException e) {
				L.e("could not send data");
                Toast. makeText ( mContext, "No se pudo enviar dato.!!!" ,
                        Toast . LENGTH_SHORT ). show (); // display a toast message
			}
		} else {
			L.d("accessory not connected");
            Toast. makeText ( mContext, "No se encontro accesorio..!!!" ,
                    Toast . LENGTH_SHORT ). show (); // display a toast message
		}
	}
